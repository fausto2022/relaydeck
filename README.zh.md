...
- 支持同步上游分组/模型倍率。
- 首页提供按 OpenAI、Anthropic、Gemini、Grok、生图和其他类型划分的倍率排行，支持按分组名称关键词配置二级分类和未命中兜底策略。
...
Docker 默认拉取 `ghcr.io/fausto2022/relaydeck:${IMAGE_TAG:-edge}`，不会在本机编译镜像。配置和数据都会写入宿主机项目目录下的 `data/`。
...
生产环境应在 `.env` 中固定 `IMAGE_TAG`。升级时将其改为目标版本，而不是长期跟随 `edge`。`APP_SECRET` 必须保持不变，否则主站 Admin API Key 和其他既有密文无法解密。
...
通知日志支持分页加载，告警动态卡片默认展示最近记录，详情弹窗可继续加载历史记录。所有通知标题统一使用固定的 `[RelayDeck]` 前缀，正文采用结构化 Markdown 富文本，方便在 Server酱、Telegram、Webhook、邮件等外部通知工具中阅读、过滤和归档。
...
旧版 `/api/upstream-sync/*` 接口已停止注册，不再执行倍率扫描后的旧同步任务。主站分组和账号统一通过 `/api/main-station/*` 管理。
...
- 主站探活、利润评估和操作记录保留 30 天。
- 倍率变化日志默认不清理。
- SQLite 部署会在启动时和每日清理任务中在线备份到 `data/backups/`，保留最近 7 份。
