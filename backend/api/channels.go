...
func registerChannels(g *gin.RouterGroup, d *Deps) {
	...
	gp.GET("/:id/rates/:rate_id/main-station-usage", func(c *gin.Context) { channelRateMainStationUsage(c, d) })
...
func channelRateMainStationUsage(c *gin.Context, d *Deps) {
	if d.MainStation == nil {
		fail(c, http.StatusServiceUnavailable, errors.New("主站服务尚未初始化"))
		return
	}
	...
	result, err := d.MainStation.GetRateUsage(channelID, rateID)
	if err != nil {
		failMainStation(c, err)
		return
	}
