...
- 可选的完整检测模型。
- 主站配置可按 OpenAI、Anthropic、Gemini、Grok 等平台设置全局探活模型，并设置全局探活间隔、连续失败停用次数和连续成功恢复次数；默认每 30 秒探活、连续失败 10 次停用、连续成功 3 次恢复。
- 单个账号可覆盖探活模型、探活间隔和失败/恢复次数，未设置时继承全局配置；账号探活间隔最低 1 秒。自动任务每个周期只执行一次有效探测，有模型时执行完整模型探测，否则执行快速连通探测。
- HTTP 429、网络错误、鉴权失败和其他有效探测失败统一累计连续失败次数，达到阈值后触发健康停用和通知；停用后继续探活，连续成功达到恢复阈值后解除健康停用。
- 账号列表显示最近 20 次有效探测的连通率，停用调度的账号仍会持续探活，只有人工关闭健康检测后才停止。
- 当前主站分组标题显示主站分组倍率。账号列表倍率列显示接管成员绑定的上游来源分组最新倍率快照，按来源分组 ID 优先、分组名兜底匹配；未指定来源分组时显示默认分组倍率 `1.00`。倍率快照变化后列表自动刷新，不再展示主站 Account 自身保存的计费倍率。
