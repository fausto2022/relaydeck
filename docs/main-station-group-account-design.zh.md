...
- 可选的完整检测模型。
- 主站配置可按 OpenAI、Anthropic、Gemini、Grok 等平台设置全局探活模型，并设置全局探活间隔；模型下拉合并 Sub2API 内置官方目录、主站 Account 同步目录和来源账号标准模型接口。
- 单个账号可覆盖探活模型和探活间隔，未设置时继承平台全局模型和全局间隔；自动任务每个周期只执行一次有效探测，有模型时执行完整模型探测，否则执行快速连通探测。
